...
static unsigned int current_pic = 0;

/*  Function for drawing a scatter plot of an array arr of length arr_length. */
static void draw_pic(const double * const arr, unsigned int arr_length)
{
    /*  Declare variables for sampling the box we're drawing to.              */
    unsigned int x, y;
    const double rcpr_factor = 1.0 / (double)arr_length;

    /*  Set the boundaries of the box.                                        */
    const double y_min = 0.0;
    const double y_max = 1.0;
    const double pixel_width = 0.001;

    /*  Variables for coloring the current pixel.                             */
    const unsigned char black = 0U;
    const unsigned char white = 255U;
    ...
    sprintf(str, "merge_sort_%02u.pgm", current_pic);
    ...
    /*  We're finished writing to the PGM file. We can close it.              */
    fclose(fp);
...
static void
merge(const double * const unsorted, unsigned left, unsigned int right,
      unsigned int end, double * const sorted)
{
...
static double *merge_sort(const double * const arr, unsigned int arr_size)
{
