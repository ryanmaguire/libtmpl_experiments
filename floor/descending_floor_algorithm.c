 ...
 *      decimals. This does not assume the IEEE 754 format is implemented,    *
 *      and is portable. It is, however, 10 times slower than glibc's floor   *
 *      function which does take advantage of the IEEE 754 format.            *
 ******************************************************************************
 *  Author:     Ryan Maguire                                                  *
 *  Date:       July 28, 2021                                                 *
 ******************************************************************************/
...
static double my_floor(double x)
{
    ...
    /*  For real numbers greater than 2^52 we can treat as integers to        *
     *  a reasonable precision.                                               */
    if (abs_x > 4503599627370496.0)
        return x;

    /*  Find the greatest power of 2 less than |x|.                           */
    n = 0U;

    while (abs_x < arr[n])
        ++n;

    /*  We can start our binary search at this power of two.                  */
    out = arr[n];
    dx = abs_x - out;
    ...
    while (dx > 1.0)
    {
        ...
        /*  If |x| > out, flip the current bit of "out" to one.               */
        if (abs_x > out)
            out += arr[n];

        /*  The previous iteration may have gone past |x|. If so, flip the    *
         *  previous bit back to zero and flip the current bit to one.        */
        else if (abs_x < out)
            out -= arr[n];
