...
long double Arcsin_LDouble_Tail_End(long double x)
{
    /*  Rational function is computed in terms of (1 - x)/2.                  */
    const long double z = 0.5L*(1.0L - x);

    /*  Use Horner's method to evaluate the two polynomials.                  */
    const long double p = NUM_EVAL(z);
    const long double q = DEN_EVAL(z);
    ...
    return PI_BY_TWO - 2.0L*(s + t);
}
/*  End of Arcsin_LDouble_Tail_End.                                           */
